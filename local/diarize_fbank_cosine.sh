   ...
   if [ $modeltype == "etdnn" ];then
     local/diarization/nnet3/xvector/extract_xvectors.sh \
       ...
       local/diarization/nnet3/xvector/extract_xvectors_pytorch.sh \
        ...
        --window 1.5 --period $period --apply-cmn false --use-gpu true \
        --min-segment 0.25 \
        $model_dir data/${name}_cmn $out_dir/xvectors_$period
   fi
fi
...
###############################################################################
##HAVE TO CHANGE THIS TO COSINE SCORING 

plda_dir=$out_dir/plda_scores
if [ $stage -le 3 ]; then
  echo "$0: Performing cosine scoring..."
  ...
  local/diarization/score_cosine.sh \
    --nj $nj --cmd "$decode_med_cmd" \
    --target-energy $target_energy \
    $out_dir/xvectors_$period $plda_dir
fi
